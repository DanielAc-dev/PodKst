...
function getAlbums (req, res){
    ...
    if(!artistId){
        ...
        //Sacar album de un artista concreto de la DB
...
function deleteAlbum(req, res){
    ...
    Album.findByIdAndRemove(albumId, (err, albumRemoved) => {
        if(err){
            res.status(500).send({message: 'Error al eliminiar album'});
        }else{
            if(!albumRemoved){
                ...
                Song.find({album: albumRemoved._id}).remove((err, songRemoved) => {
                    if(err){
                        res.status(500).send({message: 'Error al eliminiar la cancion'});
                    }else{
                        if(!songRemoved){
